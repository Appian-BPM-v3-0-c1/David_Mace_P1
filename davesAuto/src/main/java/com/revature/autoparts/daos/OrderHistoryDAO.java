...
public class OrderHistoryDAO implements CrudDAO<OrderHistory> {
    ...
    public List<OrderHistory> findByDateAsc(int id) {
        List<OrderHistory> ascList = new ArrayList<>();

        try {
            PreparedStatement ps = con.prepareStatement("SELECT * FROM order_history WHERE customer_id = (?) ORDER BY date ASC");
            ps.setInt(1,id);
    ...
    public List<OrderHistory> findByDateDesc(int id) {
        List<OrderHistory> descList = new ArrayList<>();

        try {
            PreparedStatement ps = con.prepareStatement("SELECT * FROM order_history WHERE customer_id = (?) ORDER BY date DESC");
            ps.setInt(1, id);
